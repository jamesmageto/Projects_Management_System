  ...
  <div id="container">
    <div id="links_table_container">
      <table id="links_table" border="1" cellspacing="0.6" cellpadding="5">
        ...
        <tr>
        <tr>
          <td id="upload_col"><a href="student_search.php" id="upload">Upload</a></td>
        <tr>
      </table>
    </div>
...
// if user has been granted access to download file then display link
// otherwise display link to request source code
$result1 = mysqli_query($con,"SELECT * FROM request WHERE id=$id");
if (mysqli_error($con))
{
...
if ( !strcmp($_SESSION['name'], $row1['name']) && $row1['grant1'] )
{
  ...
  echo "<a href=$path id=\"request_link\">Download source code</a>";
  echo "</div>";
  echo "<br/><br/>";
}
else
{
  ...
  else
  {
    ...
    echo "<div id=\"request_and_back\">";
    echo "<a href=$path id=\"request_link\">Request for source code</a>";
    echo "</div>";
    echo "<br/><br/>";
  }
}
echo "<div id=\"request_and_back\">";
echo "<a href=\"student_projects.php\" id=\"back\"><< Go Back</a>";
echo "</div>";
